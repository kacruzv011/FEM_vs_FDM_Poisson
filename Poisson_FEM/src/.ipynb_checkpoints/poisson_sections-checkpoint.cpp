// --- ARCHIVO: src/poisson_sections.cpp (MODO DUAL) ---
...
// Usa directivas 'atomic' y 'critical' para la sincronización.
...
// --- FUNCIÓN PRINCIPAL DE CÁLCULO (PARALELA CON SECTIONS/ATOMIC) ---
void solve_fem_sections(int M, int N, const std::vector<Node>& nodes, Eigen::VectorXd& u) {
    int n_nodes = nodes.size();
    std::vector<T> global_triplets;
    Eigen::VectorXd F = Eigen::VectorXd::Zero(n_nodes);
    
    // El vector de triplets no es seguro para accesos atómicos directos,
    // por lo que cada hilo necesitará un buffer local para ellos.
    // Sin embargo, podemos probar el acceso atómico en el vector F.
                ...
                for (const auto* tri_nodes : {tri1_nodes, tri2_nodes}) {
                    ...
                    for (int i_node = 0; i_node < 3; ++i_node) {
                        if (!nodes[tri_nodes[i_node]].is_boundary) {
                            #pragma omp atomic
                            F(tri_nodes[i_node]) += f_val;
                            for (int j_node = 0; j_node < 3; ++j_node) {
                                if (nodes[tri_nodes[j_node]].is_boundary) {
                                    #pragma omp atomic
                                    F(tri_nodes[i_node]) -= Ke(i_node, j_node) * boundary_condition(nodes[tri_nodes[j_node]].x, nodes[tri_nodes[j_node]].y);
                                } else {
    ...
    Eigen::SimplicialLLT<SpMat> solver(K);
    u = solver.solve(F);
}
...
int main(int argc, char *argv[]) {
    ...
    solve_fem_sections(M, N, nodes, u);
