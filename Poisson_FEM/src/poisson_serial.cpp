// --- ARCHIVO: src/poisson_serial.cpp (MODO DUAL) ---
...
// --- FUNCIÓN PRINCIPAL DE CÁLCULO (SERIAL) ---
void solve_fem_serial(int M, int N, const std::vector<Node>& nodes, Eigen::VectorXd& u) {
    int n_nodes = nodes.size();
    std::vector<T> global_triplets;
    Eigen::VectorXd F = Eigen::VectorXd::Zero(n_nodes);

    // Bucle sobre cada cuadrilátero de la malla
    for (int j = 0; j < N; ++j) {
        for (int i = 0; i < M; ++i) {
            int n0 = j * (M + 1) + i;
            int n1 = n0 + 1;
            ...
            // Procesar los dos triángulos del cuadrilátero
                    ...
                    if (!nodes[tri_nodes[i_node]].is_boundary) {
                        ...
                        for (int j_node = 0; j_node < 3; ++j_node) {
                            if (!nodes[tri_nodes[j_node]].is_boundary) {
                                global_triplets.emplace_back(tri_nodes[i_node], tri_nodes[j_node], Ke(i_node, j_node));
                            } else {
...
int main(int argc, char *argv[]) {
    ...
    const int num_threads = std::atoi(argv[3]); // Se recibe pero no se usa
