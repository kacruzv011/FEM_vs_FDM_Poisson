// --- ARCHIVO: src/poisson_schedule.cpp (FDM Refactorizado - MODO DUAL) ---
...
// Paralelizado con OpenMP usando schedule(static) o schedule(dynamic).
...
void solve_poisson_fdm_schedule(int M, int N, int schedule_type, std::vector<std::vector<double>> &V, double h, double k) {
    ...
    while (delta > TOLERANCE && iterations < MAX_ITERATIONS) {
        ...
        if (schedule_type == 1) { // static
            #pragma omp parallel for collapse(2) reduction(max:delta) schedule(static)
            for (int i = 1; i < M; ++i) {
                for (int j = 1; j < N; ++j) {
                    double x = x_ini + i * h;
                    double y = y_ini + j * k;
                    double f = source_term(x, y);
                    double numer = (V_old[i+1][j] + V_old[i-1][j]) * (k*k) + (V_old[i][j+1] + V_old[i][j-1]) * (h*h) - f * h2k2;
            ...
            #pragma omp parallel for collapse(2) reduction(max:delta) schedule(dynamic)
            for (int i = 1; i < M; ++i) {
                for (int j = 1; j < N; ++j) {
                    double x = x_ini + i * h;
                    double y = y_ini + j * k;
...
int main(int argc, char *argv[]) {
    ...
    const int schedule_type = std::atoi(argv[5]);
    ...
    if (schedule_type != 1 && schedule_type != 2) { std::cerr << "Schedule no válido\n"; return 1; }
    ...
    solve_poisson_fdm_schedule(M, N, schedule_type, V, h, k);
    ...
    if (mode == "solution") {
        ...
        // En modo benchmark, la salida incluye el tipo de schedule
        std::cout << M << "," << N << "," << num_threads << "," << schedule_type << "," << (end_time - start_time) << std::endl;
    }
    return 0;
}