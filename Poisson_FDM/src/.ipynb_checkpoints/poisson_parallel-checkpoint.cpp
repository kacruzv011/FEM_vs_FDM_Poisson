// --- ARCHIVO: src/poisson_parallel.cpp (FDM Refactorizado - MODO DUAL) ---
...
// Paralelizado con OpenMP, itera hasta convergencia.
...
static constexpr int MAX_ITERATIONS = 50000000; // Límite de seguridad
...
void initialize_grid(int M, int N, std::vector<std::vector<double>> &V, double h, double k) {
    ...
    // Se puede paralelizar la inicialización para mallas muy grandes
...
void solve_poisson_fdm_parallel(int M, int N, std::vector<std::vector<double>> &V, double h, double k) {
    ...
    while (delta > TOLERANCE && iterations < MAX_ITERATIONS) {
        delta = 0.0;
        V_old = V; // Copia la matriz para la lectura de la iteración anterior (Método de Jacobi)
        ...
        for (int i = 1; i < M; ++i) {
            for (int j = 1; j < N; ++j) {
                ...
                // La reducción max se encarga de encontrar el máximo delta entre todos los hilos
                if (diff > delta) {
                    delta = diff;
                }
            }
...
int main(int argc, char *argv[]) {
    ...
    solve_poisson_fdm_parallel(M, N, V, h, k);
